...
```
- 에상 출력 : y = {10, 9, 7, 13, 9, 5, 10}
  - self-modifying code로 배열 주소를 매 반복마다 +1
  - loop + BRZ 카운터로 7회 반복
...
```
PC=0x10 LDA R1,[x+n+1] <- 0x22에서 수정됨
PC=0x11 LDA R2,R0 (m=1) -> R2=x[n]
PC=0x12 ADD R2,R1 (m=1) -> R2=x[n]+x[n+1]
PC=0x13 STA [y+n],R2 <- 0x32에서 수정됨
PC=0x14 LDA R0,R1 (m=1) -> R0 shift
PC=0x15 BRA 0x20
```
...
```
```
PC=0x30 LDA R2,[0x13] -> 명령어 읽기
PC=0x31 ADD R2,[0xA1] -> addr +1
PC=0x32 STA [0x13],R2 -> 명령어 갱신
...
100_0_10_00_10010000    //  0x13  STA [0x90],R2     y[0] <- R2 (0x32 Update)
