...
- MicroCPU의 구성 블럭과 모듈/인스턴스 구조를 파악한다
...
- 2.1: 9개 블럭이 단일 clk_sys로 동작. 모듈명/인스턴스명 체계
- 2.2: cpu_pkg — opcode_t(8개), state_t(8상태) enum 정의
- 2.3: sysclk — clock gating + 2분주. halt 시 전체 정지
- 2.4: mem — 256x16 동기 메모리. read/write enable 제어
- 2.5: Controller — 8-상태 FSM. op_memrd + is_not 분류로 8개 제어 신호 생성
- 2.6: IR — 16비트 래치 + 5개 필드 디코딩
- 2.7: regfile — 4x16b, 조합 읽기 + 동기 쓰기
- 2.8~2.10: MUX, PC — 조합 선택, 카운터
- 2.11: ALU — 조합 연산. ADD, AND, NOT, LDA
